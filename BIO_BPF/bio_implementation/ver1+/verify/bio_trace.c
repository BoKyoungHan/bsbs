...
struct data_t {
	...
	u64 time;
	u64 request;
	unsigned int data_size_start;
	unsigned int data_size_end;
	u32 PID1;
	u32 PID2;
...
void submit_bio_entry(struct pt_regs *ctx, struct bio * bio){
	...
	u64 ptr_bio = (u64)bio;
        struct blk_layer_called_info blk_layer_called_info = {
                .start = start_time,
                .end = 0,
		.s_size = bio->bi_iter.bi_size,
		.e_size = 0,
		.PID1 = bpf_get_current_pid_tgid(),
		.PID2 = 0
        };
...
//void dio_complete_entry(struct pt_regs *ctx, struct dio * dio){
	...
	u64 start_time = 0;
        ...
        blk_layer_called_info = blk_layer_called_info_map.lookup( &ptr_bio);
        if(!blk_layer_called_info){
                bpf_trace_printk("Bio is not registered at blk_layer_called_info_map\n");
                return ;
        }
	...
	blk_layer_called_info->e_size = bio->bi_iter.bi_size;
	blk_layer_called_info->PID2 = bpf_get_current_pid_tgid();
	...
	data.data_size_start = blk_layer_called_info->s_size;
	data.data_size_end = blk_layer_called_info->e_size;
	data.PID1 =  blk_layer_called_info->PID1;
	data.PID2 =  blk_layer_called_info->PID2;
	events.perf_submit(ctx,&data,sizeof(data));

	blk_layer_called_info_map.delete( &ptr_bio);
	bio_to_request_info_map.delete(&ptr_bio);
